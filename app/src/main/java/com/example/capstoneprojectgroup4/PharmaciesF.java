...
public class PharmaciesF extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        databaseB = v.findViewById(R.id.database_b);
        sentenceT = v.findViewById(R.id.sentence_t);

        database = FirebaseDatabase.getInstance();
        DatabaseReference myRef = database.getReference("message");
        myRef.setValue("Hello, World!");

        databaseB.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String sentence = sentenceT.getText().toString();

                if(TextUtils.isEmpty(sentence)){
                    Toast.makeText(getActivity(), "Text field cannot be empty.", Toast.LENGTH_SHORT).show();
                    return;
                }
